...
export default function BottomNav({ profile, view, onView, onProfiles }) {
  ...
  return (
    <nav className="bottom-nav" aria-label="Main navigation">
      <button className={view === "home" ? "active" : ""} onClick={() => onView("home")} type="button"><Home size={21} /><span>Home</span></button>
      {isChild && <button className={view === "rewards" ? "active" : ""} onClick={() => onView("rewards")} type="button"><Gift size={21} /><span>Rewards</span></button>}
